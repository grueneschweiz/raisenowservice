...
class RaisenowPaymentData
{
    ...
    public static function fromRequestData(): RaisenowPaymentData
    {
        $payment = new RaisenowPaymentData(
            mb_strtolower(self::getPostData('language')),
            mb_strtolower(self::getPostData('stored_customer_salutation')),
            self::getPostData('stored_customer_firstname'),
            self::getPostData('stored_customer_lastname'),
            self::getPostData('stored_customer_company'),
            self::getAddress1(),
            self::getAddress2(),
            self::getPostData('stored_customer_zip_code'),
            self::getPostData('stored_customer_city'),
            mb_strtoupper(self::getPostData('stored_customer_country')),
            self::getPostData('stored_customer_email'),
            (int) filter_var(self::getPostData('amount'), FILTER_VALIDATE_INT),
            date_create_immutable(self::getPostData('created')),
            self::getPostData('epp_transaction_id'),
            self::getPostData('stored_rnw_source_url'),
            self::getPostData('payment_method'),
            self::getNewsletter(),
            self::getPostData('stored_customer_message'),
        );
    ...
    private function validate(): void
    {
        if ( ! in_array($this->language, ['de', 'fr', 'it', 'en'])) {
            throw new RaisenowPaymentDataException("Invalid payment data. Given language is not valid: $this->language",
                $this);
        }

        if ( ! in_array($this->salutation, ['mr', 'ms', 'neutral'])) {
            throw new RaisenowPaymentDataException("Invalid payment data. Given salutation is not valid: $this->salutation",
                $this);
        }
        ...
        if ( ! in_array($this->country, ['CH', 'DE', 'FR', 'IT', 'AT'])) {
            ...
            throw new RaisenowPaymentDataException("Invalid payment data. Payment from unknown country: $this->country",
                $this);
        }

        if ( ! filter_var($this->email, FILTER_VALIDATE_EMAIL)) {
            throw new RaisenowPaymentDataException("Invalid payment data. Given email address is not valid: $this->email",
                $this);
        }

        if ( ! $this->created) {
            throw new RaisenowPaymentDataException("Invalid payment data. Failed to parse date: {'created']}",
                $this);
        }
