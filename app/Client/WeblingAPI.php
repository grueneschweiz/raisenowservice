...
class WeblingAPI
{
    ...
    public function paymentExists(RaisenowPaymentData $payment): bool
    {
        ...
        try {
            ...
            throw new WeblingAPIException(
                "Invalid response from debitor endpoint of Webling. Response: {$response->getBody()}."
            );
        }
    ...
    private function getPeriodId(DateTimeInterface $date): int
    {
        ...
        try {
            ...
            throw new WeblingAPIException(
                "Invalid response from period endpoint of Webling. Response: {$response->getBody()}."
            );
        }
    ...
    private function obtainAccountingConfig(DateTimeInterface $date): WeblingAccountingConfig
    {
        ...
        $filename = Config::name() . "-$periodId-" . md5(
                "$donationAccountTemplateId|$debtorAccountTemplateId|$bankAccountTemplateId"
            );
    ...
    private function getAccountId(int $periodId, int $accountTemplateId): int
    {
        ...
        try {
            ...
            throw new WeblingAPIException(
                "Invalid response from account endpoint of Webling. Response: {$response->getBody()}."
            );
        }
        ...
        if (count($data['objects']) !== 1) {
            throw new WeblingAPIException(
                "No account found in Webling for periodId $periodId and accountTemplateId $accountTemplateId"
            );
        }
